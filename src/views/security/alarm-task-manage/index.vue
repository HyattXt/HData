            ...
            <div class="cue-table">
              <div class="cue-table-container">
                <el-table v-loading="loadingRef" :data="dataRef" show-overflow-tooltip border resizable highlight-current-row @current-change="handleCurrentChange" height="100%">
                  ...
                  <el-table-column prop="alarmContent" label="告警描述" align="center"/>
    ...
    <div class="add-dialog">
      ...
      <el-dialog v-model="activeAdd" width="800px" append-to-body :before-close="dialogFormVisible">
        ...
        <n-grid :cols="24" :x-gap="24">
          ...
          <n-form-item-gi :span="12" label="告警描述:" path="alarmDescription">
          ...
          <n-form-item-gi :span="24" label="全量通知内容:" path="alarmContent">
            <n-input v-model:value="formValues.alarmContent" />
          </n-form-item-gi>
          <n-form-item-gi :span="24" label="成功通知内容:" path="alarmContentSuccess">
            <n-input v-model:value="formValues.alarmContentSuccess" />
          </n-form-item-gi>
          <n-form-item-gi :span="24" label="失败通知内容:" path="alarmContentFailed">
            <n-input v-model:value="formValues.alarmContentFailed" />
          </n-form-item-gi>
        </n-grid>
        </n-form>
        <template #footer>
...
const rules = reactive({
  ...
  alarmContent: {
    validator: (rule, value, callback) => {
      if (formValues.value.taskAlarmStatus === 3 && !value) {
        callback(new Error('全量通知内容不能为空'));
      } else {
        callback();
      }
    },
  ...
  alarmContentSuccess: {
    validator: (rule, value, callback) => {
      if (formValues.value.taskAlarmStatus === 1 && !value) {
        callback(new Error('成功通知内容不能为空'));
      } else {
        callback();
      }
    },
  ...
  alarmContentFailed: {
    validator: (rule, value, callback) => {
      if (formValues.value.taskAlarmStatus === 2 && !value) {
        callback(new Error('失败通知内容不能为空'));
      } else {
        callback();
      }
    },
...
function addMetadata() {
  detailPaginationReactive.page = 1; detailPaginationReactive.taskName= ''; detailPaginationReactive.processName= '';
  detailQuery(
...
const delConfirm = () => {
  ElMessageBox.confirm(
      ...
      .then(() => {
        delMetadata(currentRow.value.id, currentRow.value.taskName, currentRow.value.dataCatalogAddFlag, currentRow.value.dataSourceAddFlag)
      })
      .catch(() => {
      })
}
...
const submitFormAdd = () => {
  ruleFormRef.value.validate(async (errors) => {
    if (!errors) {
      ...
      let url = ''
      url = formValues.value.operate === '新增' ? utils.getUrl('Alarm/insert') : utils.getUrl('Alarm/update')
      axios
        .post(url, params)
        .then(function (response) {

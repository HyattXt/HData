...
const getResources = async () => {
  if (taskData.value.resourcesLoading) return
  taskData.value.resourcesLoading = true
  const res = await queryResourceList({ type: 'FILE' })
  utils.removeUselessChildren(res)
  taskData.value.resourcesOptions = res || []
  taskData.value.resourcesLoading = false
}

const json = computed(() => {
  return [...Fields.useSeaTunnel(taskData.value)];
});

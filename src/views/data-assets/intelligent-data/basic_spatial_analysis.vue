            ...
            <div class="cue-table">
              <div class="cue-table-container">
                <el-table v-loading="loadingRef" :data="TableData.tableList" border resizable highlight-current-row height="100%" show-overflow-tooltip @current-change="handleCurrentChange">
                  ...
                  <el-table-column prop="spaceMeasurementThreshold" label="空间测量阈值" align="center"/>
                  <el-table-column prop="spatialQueryMethod" label="空间查询方法" align="center"/>
                  <el-table-column prop="topologyAnalysisMethod" label="拓扑分析方法" align="center"/>
                  <el-table-column prop="connectivityAnalysisMethod" label="连通性分析方法" align="center"/>
                </el-table>
              </div>
            </div>
      ...
      <template #header> {{ indexFormValue.opperate }} </template>
        ...
        >
          <n-grid :cols="24" :x-gap="24">
            ...
            <n-form-item-gi :span="12" label="空间测量阈值:" path="spaceMeasurementThreshold">
              <n-input v-model:value="indexFormValue.spaceMeasurementThreshold" placeholder="请输入空间测量阈值" :disabled="ifUpdate"/>
            </n-form-item-gi>
          </n-grid>
          <n-grid :cols="24" :x-gap="24">
            <n-form-item-gi :span="12" label="空间查询方法:" path="spatialQueryMethod">
              <n-input v-model:value="indexFormValue.spatialQueryMethod" placeholder="请输入空间查询方法" :disabled="ifUpdate"/>
            </n-form-item-gi>
            <n-form-item-gi :span="12" label="拓扑分析方法:" path="topologyAnalysisMethod">
              <n-input v-model:value="indexFormValue.topologyAnalysisMethod" placeholder="请输入拓扑分析方法" :disabled="ifUpdate"/>
            </n-form-item-gi>
          </n-grid>
          <n-grid :cols="24" :x-gap="24">
            <n-form-item-gi :span="12" label="连通性分析方法:" path="connectivityAnalysisMethod">
              <n-input v-model:value="indexFormValue.connectivityAnalysisMethod" placeholder="请输入连通性分析方法" :disabled="ifUpdate"/>
            </n-form-item-gi>
          </n-grid>
        </n-form>
        <template #footer>
...
const insertIndexUrl = utils.getUrl('QthBasicSpatialAnalysis/insert')
const updateIndexUrl = utils.getUrl('QthBasicSpatialAnalysis/update')
const deleteIndexUrl = utils.getUrl('QthBasicSpatialAnalysis/delete')
...
const paginationReactive = reactive({
  spatialDataName: '',
  spaceMeasurementThreshold: '',
  page: 1,
  pageSize: 30,
  spatialQueryMethod: '',
  topologyAnalysisMethod: '',
  connectivityAnalysisMethod: '',
  itemCount: 0
})
...
function query(
  ...
  const url = utils.getUrl('QthBasicSpatialAnalysis/getList')
  const params = {
    'pageNum': page,
    'pageSize': pageSize,
...
function exportToCSV() {
  ...
  const header = ['序号', '空间数据名称', '空间测量阈值', '空间查询方法', '拓扑分析方法', '连通性分析方法']

<template>
  ...
  >
    <n-grid :cols="24" :x-gap="24">
      <n-form-item-gi :span="24" label="服务名称" path="apiName">
        <n-input v-model:value="formValue.apiName" placeholder="请输入" />
      </n-form-item-gi>
      <n-form-item-gi :span="24" label="服务Host" path="apiIpaddr">
        <n-input v-model:value="formValue.apiIpaddr" :disabled="isDisable" placeholder="以http://开头，且不包含path" />
      </n-form-item-gi>
      <n-form-item-gi :span="24" label="服务Path" path="apiPath">
        <n-input v-model:value="formValue.apiPath" :disabled="isDisable" placeholder="以/开头" />
      </n-form-item-gi>
      ...
      <n-form-item-gi :span="12" label="请求体格式" v-if="formValue.apiMethod && formValue.apiMethod !== 'GET'">
        <n-select
      ...
      <n-form-item-gi :span="12" label="来源">
        <n-input v-model:value="formValue.apiDataSource" placeholder="请输入" />
      </n-form-item-gi>
      <n-form-item-gi :span="12" label="频次限制" path="apiFrequency">
        <n-input-group>
          <n-input-number v-model:value="formValue.apiFrequency" placeholder="请输入" :style="{ width: '100%' }" />
          <n-input-group-label>次/秒</n-input-group-label>
        </n-input-group>
      </n-form-item-gi>
      <n-form-item-gi :span="24" label="描述" path="apiComment">
        <n-input v-model:value="formValue.apiComment" type="textarea" :rows="2" placeholder="描述" />
      </n-form-item-gi>
      <n-form-item-gi :span="24" label="是否请求token">
        <n-switch v-model:value="formValue.dynamicTokenState" :checked-value="1" :unchecked-value="2" />
      </n-form-item-gi>
      <n-form-item-gi :span="formValue.dynamicTokenState === 1 ? 12 : 0" label="token位置" path="dynamicTokenPosition">
        <n-select
          v-model:value="formValue.dynamicTokenPosition"
          :options="[
      ...
      <n-form-item-gi :span="formValue.dynamicTokenState === 1 ? 12 : 0" label="接口类型" path="dynamicTokenHttpType">
        <n-select
          v-model:value="formValue.dynamicTokenHttpType"
          :options="[
            { label: 'POST', value: 1 },
            { label: 'GET', value: 2 }
          ]"
      ...
      <n-form-item-gi
        :span="formValue.dynamicTokenState === 1 ? 12 : 0"
        label="接口请求头"
        path="dynamicTokenHeaderParam"
      >
  ...
  function getInitData() {
    ...
    apiAxios
      ...
      .then(function (response) {
        ...
        let tmpHeaderParams = response.data.obj.headersArray
          .filter((item) => item.paramTitle !== 'HDataApiToken')
          .map((item) => {
